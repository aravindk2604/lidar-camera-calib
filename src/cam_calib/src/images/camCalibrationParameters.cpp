...
int main() {
  ...
  for (int k = 1; k <= numOfImages; k++) {
    ...
    for display. imshow( "Display window", grayImage ); waitKey(0);
    ...
    */
    ...
    //	cout << endl << obj3D.size() << "___" << objectCoordinates.size() <<
    //"___" << corners.size() <<  "___" << imageCoordinates.size() << endl;
  }
  ...
  calibrateCamera(objectCoordinates, imageCoordinates, clrImage.size(), K, D,
                  rvecs, tvecs, flag);

  cout << "Calibration error: "
       << computeReprojectionErrors(objectCoordinates, imageCoordinates, rvecs,
                                    tvecs, K, D)
       << endl;
