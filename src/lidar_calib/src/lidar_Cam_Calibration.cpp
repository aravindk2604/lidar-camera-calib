...
static const std::string IMG_RECT_TOPIC = "/sensors/camera/image_color";
...
public:
    ImageConverter() : it_(nh) { 
        ...
        // Subscibe to the rosbag video
        image_sub_ = it_.subscribe(IMG_RECT_TOPIC, 1,
...
// Destructor
    ...
    void imageCb(const sensor_msgs::ImageConstPtr& msg) {
        ...
        for(size_t i = 0; i< projectedPoints.size(); ++i) {
            if(projectedPoints[i].x >= 0 && projectedPoints[i].x < IMAGE_HEIGHT && projectedPoints[i].y >= 0 && projectedPoints[i].y <IMAGE_WIDTH)
            {
